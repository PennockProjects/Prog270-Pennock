...
function loadContent(request, response) {
	...
	if (getExtension(path) === 'css') {
		...
		response.write(html);
		response.end();
	} else if ((getExtension(path) === 'png') || (getExtension(path) === 'jpg')) {
		ext = getExtension(path);
		fs.readFile(__dirname + path, "binary", function(err, file) {
			console.log("PNG or JPG detected");
			if(err) {
				...
				console.log("PNG or JPG loaded");
				response.writeHeader(200, {"Content-Type": "image/"+ext});
